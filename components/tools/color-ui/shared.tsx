...
export function ColorUiToolShell({
  title,
  subtitle,
  children,
}: {
  ...
  return (
    <section className="py-10 sm:py-14 md:py-16 relative overflow-hidden">
      <div
        className="absolute top-0 left-1/3 w-96 h-96 opacity-10 blur-3xl pointer-events-none"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <RevealGroup stagger={70}>
          <RevealItem>
            <SectionHeading badge="// Color & UI" title={title} subtitle={subtitle} />
          </RevealItem>
          {children}
        </RevealGroup>
      </div>
...
export function OutputBlock({
  ...
  return (
    <pre
      className={cn(
        "min-h-[120px] max-h-[420px] overflow-auto p-3 sm:p-4 border border-border/60 bg-background/50 font-mono text-xs sm:text-sm leading-relaxed whitespace-pre-wrap break-words",
...
export function ColorSwatch({
  ...
  return (
    <div
      className={cn("border border-border/60 cyber-chamfer-sm", className)}
